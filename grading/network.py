...
self.classif ier= nn.Linear(encoder_channels, num_classes)

def forward(self, x):
        x= self.global_pool(x)
x= x.view(x.size(0),-1)
x= self.dropout(x)
x= self.classif ier(x)
return x


class RetinoClassif ier(nn.Module):
    def __init__(self, encoder_name="resnet34", num_classes=5):
        super().__init__()
self.segmentation_model= smp.Segfor mer(
...
class EfficientNetClassif ier(nn.Module):
...
self.classif ier= nn.Sequential(
nn.Dropout(0.5),
...
return self.classif ier(features)


class ResNetClassif ier(nn.Module):
...
class HRNetClassif ier(nn.Module):
...
if"efficientnet"inmodel_name:
        model= EfficientNetClassif ier(model_name, num_classes, pretrained= pretrained)
elif"resnet"inmodel_name:
        model= ResNetClassif ier(model_name, num_classes, pretrained= pretrained)
elif"hrnet"inmodel_name:
        model= HRNetClassif ier(model_name, num_classes, pretrained= pretrained)
else:
        model= RetinoClassif ier(model_name, num_classes)
