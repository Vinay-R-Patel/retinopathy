...
def create_criterion(cfg):
    loss_config = cfg.loss.loss if hasattr(cfg.loss, 'loss') else cfg.loss
    loss_name = loss_config.name
    num_classes = getattr(cfg.model, 'num_classes', 5)

    if loss_name == "crossentropy":
        if hasattr(loss_config, 'label_smoothing') and loss_config.label_smoothing > 0:
            criterion = LabelSmoothingCrossEntropy(
                smoothing=loss_config.label_smoothing,
                num_classes=num_classes
            )
        elif hasattr(loss_config, 'class_weights') and loss_config.class_weights:
