...
def multiclass_dice_loss_differentiable(pred, target, num_classes=3, smooth=1):
    """Differentiable version of dice loss that preserves gradients"""
    pred = torch.softmax(pred, dim=1)
    total_loss = 0
...
def multiclass_dice_loss(pred, target, num_classes=3, smooth=1):
    return multiclass_dice_loss_differentiable(pred, target, num_classes, smooth)
...
class FocalDiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        focal_loss = self.focal(pred, target)
        dice_loss = multiclass_dice_loss_differentiable(pred, target, self.num_classes)
...
class DiceCrossEntropyLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        dice_loss_val = multiclass_dice_loss_differentiable(pred, target, self.num_classes)
...
def create_criterion(cfg):
    if cfg.loss.name == "combined":
        ...
        criterion = lambda pred, target: multiclass_dice_loss_differentiable(pred, target, cfg.model.num_classes)
